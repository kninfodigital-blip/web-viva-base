...
chk "enlaces de llms.txt responden 200"                "! curl -s https://$D/llms.txt | grep -o 'https://[^) ]*' | sort -u | while read u; do c=\$(curl -s -o /dev/null -m 20 -w '%{http_code}' \"\$u\"); [ \"\$c\" = 200 ] || echo \"\$c \$u\"; done | grep -q ."
